	...
	public static void resumenEstadoPruebas(HashMap<String, Integer> listaPasa,HashMap<String, Integer> listaFalla) {
		...
		for (int i = 0; i < XMLHelper.preObjects.size(); i++) {
			...
			if (estado.equals("Pasa")) {
				Integer count = listaPasa.get(preObjects.get(i).obj.getHost());
				if (count == null) {
					listaPasa.put(preObjects.get(i).obj.getHost(), 1);
				} else {
					listaPasa.put(preObjects.get(i).obj.getHost(), count + 1);
				};

			} else {
				Integer count = listaFalla.get(preObjects.get(i).obj.getHost());
				if (count == null) {
					listaFalla.put(preObjects.get(i).obj.getHost(), 1);
				} else {
					listaFalla.put(preObjects.get(i).obj.getHost(), count + 1);
				};
			}
		}
